...
```shell
npx tsx bin/artisan.ts serve                # http://127.0.0.1:8000
APP_PORT=3000 npx tsx bin/artisan.ts serve  # custom port
```
...
```shell
cd ~/sites/blog && npx tsx bin/artisan.ts serve --port=8001 &
cd ~/site/shop  && npx tsx bin/artisan.ts serve --port=8002 &
```

For production deployments see [deployment](/framework/docs/deployment); the application runs behind any process supervisor with `npx tsx bin/artisan.ts serve` as the start command.
