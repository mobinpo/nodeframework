...
function toolApplicationInfo(basePath: string): ToolResult {
    ...
    const routeFiles = fs.existsSync(routesDir)
        ? fs.readdirSync(routesDir).filter((f) => f.endsWith('.ts') || f.endsWith('.js'))
        : [];
...
function bootAppForTools(basePath: string): Promise<any> {
    // Boot lazily and cache; container-backed tools share this instance.
    const globalAny = globalThis as any;
    if (!globalAny.__NODEVEL_BOOST_APP_PROMISE) {
        globalAny.__NODEVEL_BOOST_APP_PROMISE = (async () => {
    ...
    return globalAny.__NODEVEL_BOOST_APP_PROMISE;
}

async function toolRouteList(basePath: string): Promise<ToolResult[]> {
    const app = await bootAppForTools(basePath);
    return app.make('router').getRoutes().map((route: any) => ({
...
async function toolDatabaseSchema(basePath: string, args: { table?: string }): Promise<Record<string, any[]>> {
    const app = await bootAppForTools(basePath);
    const connection = app.make('db').connection();
    const wanted: string | null = args.table || null;
    const tables: string[] = (await connection.tables()).filter((t: string) => !wanted || t === wanted);
    ...
    for (const table of tables) {
        ...
        schema[table] = columns.map((c: any) => ({ name: c.name ?? c.cid, type: c.type ?? '' }));
    }
    return schema;
}

function toolSearchDocs(basePath: string, args: { query?: string }): ToolResult {
    const needle = String(args.query || '').toLowerCase();
    if (!needle) throw new Error('A non-empty query is required.');
    const docsDir = path.join(basePath, 'docs');
    const results: { page: string; line: number; text: string }[] = [];
...
function toolLastError(basePath: string): ToolResult {
...
async function callTool(name: string, args: Record<string, any> = {}, basePath: string): Promise<ToolResult | ToolResult[]> {
    switch (name) {
...
interface JsonRpcMessage {
    ...
    method?: string;
    params?: { name?: string; arguments?: Record<string, any>; protocolVersion?: string };
}
...
function handleMessage(line: string, basePath: string): void {
    ...
    switch (method) {
        ...
        case 'tools/call':
            Promise.resolve()
                .then(() => callTool(params!.name as string, (params && params.arguments) || {}, basePath))
...
function runMcpServer(basePath: string): void {
    let buffer = '';
